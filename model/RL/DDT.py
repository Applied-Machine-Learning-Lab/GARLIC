...
class DDT(nn.Module):
    ...
    def forward(self, states, actions, returns_to_go, timesteps, attention_mask=None):
        ...
        # tmp_v = torch.sigmoid(action_preds[:, :, 0])
        # while torch.min(action_preds).item() < 0:
        #     tmp_v2 = (action_preds[:, :, 1] + 1) % 1
        #     action_preds = torch.cat([tmp_v.unsqueeze(2), tmp_v2.unsqueeze(2)], dim=2)
        # print(action_preds.shape)
